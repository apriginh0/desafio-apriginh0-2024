class RecintosZoo {
    ...
    analisaRecintos(especie, quantidade) {
      ...
      for (const recinto of this.recintos) {
        const biomasRecinto = recinto.bioma.split(' e ');  // Suporta recintos com múltiplos biomas

        if (biomasRecinto.some(bioma => animal.biomas.includes(bioma))) {
          // Verificar se é carnívoro e se há outra espécie no recinto
          ...
          if (recinto.animais.length > 0 && recinto.animais[0].especie !== especie) {
              espacoExtra = 1; // Desconta o extra corretamente
          }
          ...
          if (animal.carnivoro && animaisExistentes.length > 0) continue; // Carnívoros só com a mesma espécie
          if (temCarnivoro && animaisExistentes.length > 0 && animaisExistentes[0].especie !== especie) continue;

          // Verifica regra especial do hipopótamo
          if (especie === 'HIPOPOTAMO') {
            if (recinto.bioma !== 'savana e rio') continue;
            // O cálculo de espacoDisponivel para hipopótamo já foi ajustado acima
        }
